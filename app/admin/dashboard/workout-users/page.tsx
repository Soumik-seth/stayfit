...
export default function WorkoutUsersPage() {
  const [workoutUsers, setWorkoutUsers] = useState<WorkoutUser[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState("");
  const [selectedUser, setSelectedUser] =
    useState<WorkoutUser | null>(null);

  const fetchWorkoutUsers = async () => {
    try {
      setLoading(true);

      const response = await fetch(
        "/api/admin/workout-users",
        {
          cache: "no-store",
        }
      );
      ...
      if (!response.ok) {
        alert(data.message || "Failed to load workout users.");
        return;
      }

      setWorkoutUsers(data.workoutUsers || []);
    } catch (error) {
      console.error(error);
  ...
  const filteredUsers = useMemo(() => {
    ...
    if (!value) return workoutUsers;

    return workoutUsers.filter((item) => {
      return (
        item.user.fullName
          .toLowerCase()
          .includes(value) ||
  ...
  const activeUsers = workoutUsers.filter(
    (item) => item.status === "ACTIVE"
  ).length;

  const expiredUsers = workoutUsers.filter(
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 sm:p-6 lg:p-8">
      ...
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-[#0C4372] sm:text-3xl">
            Manage Workout Users
          </h1>

          <p className="mt-1 text-sm text-gray-500">
            Manage users who have purchased a workout plan.
          </p>
        </div>

        <button
          onClick={fetchWorkoutUsers}
      ...
      <div className="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
        ...
        <StatCard
          title="Total Workout Users"
          value={workoutUsers.length}
          icon={<Users size={22} />}
        />
      ...
      {loading ? (
        ...
        <div className="rounded-xl bg-white p-10 text-center shadow-sm">
          ...
          <h3 className="mt-4 font-semibold text-gray-700">
            No workout users found
          </h3>

          <p className="mt-1 text-sm text-gray-500">
            {search
              ? "Try a different search."
              : "No user has purchased a workout plan yet."}
          </p>
                  ...
                  {filteredUsers.map((item) => (
                    ...
                    >
                      ...
                      <td className="px-5 py-4 text-sm text-gray-600">
                        {item.user.phoneNumber || "Not provided"}
                      </td>
          ...
          <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white shadow-xl">
            ...
            <div className="space-y-6 p-5">
              ...
              <section>
                ...
                <h3 className="mb-3 font-semibold text-gray-800">
                  Workout Subscription Information
                </h3>
